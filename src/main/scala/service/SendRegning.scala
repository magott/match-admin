...
object SendRegning {
  ...
  def create : Option[SendRegning] = {
    val postnrMap = scala.io.Source.fromInputStream(getClass.getResourceAsStream("/Postnummerregister-ansi.txt")).getLines()
          .map{ s =>
    ...
    for {
      ...
      postnrMap <- Try(Source.fromInputStream(getClass.getResourceAsStream("/Postnummerregister-ansi.txt")).getLines()
        .map{ s =>
          val splitted = s.split("\t")
          splitted(0) -> splitted(1)
        }.toMap).toOption
