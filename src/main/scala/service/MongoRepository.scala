...
import com.mongodb.casbah.{MongoDB, WriteConcern}
...
class MongoRepository(db: MongoDB) extends SessionRepository {
  ...
  def refInterestedInMatch(matchId: ObjectId, userId: ObjectId): Boolean     = {
    val user    = User.fromMongo(db("users").findOneByID(userId).get)
    val updated =
      db("matches").update(q = where("_id" -> matchId), o = $addToSet("intRef" -> Referee.fromUser(user).toMongo), concern = WriteConcern.Acknowledged).getN
    updated == 1
  }
...
//    db("foo").update(q= where("_id" -> id), o= $pull(where("number" -> MongoDBObject("num" -> 2))))
    db("matches")
      .update(q = where("_id" -> matchId), o = $pull(where("intRef" -> has("_id" -> refId))), upsert = true, multi = true, concern = WriteConcern.Acknowledged)
      .getN > 0
  }
...
//    db("foo").update(q= where("_id" -> id), o= $pull(where("number" -> MongoDBObject("num" -> 2))))
    db("matches")
      .update(q = where("_id" -> matchId), o = $pull(where("intAss" -> has("_id" -> refId))), upsert = true, multi = true, concern = WriteConcern.Acknowledged)
      .getN > 0
  }
